...
bool readClient(SOCKET socket, std::stringstream &&data)
{
    ...
    if (response.lookupResponse(std::move(data), responseInfo, answer))
    {
        ...
        case ProtocolId::Hello:
        {
            ...
            return pClient[num]->sendLogin(sessData[ch].mLogin, sessData[ch].mPassword);
        }
        break;
        case ProtocolId::Login:
            ...
            if (responseInfo.hasResponse())
            {
                ...
                return pClient[num]->sendMessage(u8R"(Test message.)");
            }
            else
        ...
        case ProtocolId::SuperMessage:
            if (responseInfo.hasResponse())
                std::cout << std::endl << "Super Message Ok" << std::endl;
            else
                std::cout << std::endl << "Super Message Fail" << std::endl;

            pClient[num]->sendSuperMessage();
